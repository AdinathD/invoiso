...
export const ProductListTable: React.FC<ProductListTableProps> = ({
  ...
  const headers = [
    ...
    {
      key: 'name',
      className: `${visibleOptionalCount === 0 ? 'w-[55%]' :
          ...
          {items.map((item, idx) => {
            ...
            return (
              <tr
                key={item.srNo}
                role="row"
                className={`border-b text-table-body font-medium transition-colors duration-150 hover:bg-app-bg/50 border-border-sec text-text-main odd:bg-panel-bg even:bg-app-bg/25 ${isFlashing ? 'animate-row-flash' : ''
                  }`}
              >
